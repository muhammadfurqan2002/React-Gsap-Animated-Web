import { useGSAP } from "@gsap/react";
import gsap from "gsap";
import React, { useContext, useRef } from "react";
import NavContext, { NavbarContext } from "../context/NavContext";

function FullScreenNav() {
  const [navOpen, setNavOpen] = useContext(NavbarContext);
  const fullNavLinksRef = useRef(null);
  const fullScreenRef = useRef(null);

  function gsapAnimation() {
    const tl = gsap.timeline();
    tl.from(".stairs", {
      height: 0,
      delay: 0.5,
      stagger: -0.2,
    });

    tl.from(".link", {
      rotateX: 90,
      opacity: 0,
      stagger: 0.3,
    });

    tl.from(".navlink", {
      opacity: 0,
    });
  }
  useGSAP(
    function () {
      if (navOpen) {
        gsap.to("#fullscreennav", {
          display: "block",
        });

        gsapAnimation();
      } else {
        gsap.to("#fullscreennav", {
          display: "none",
        });
      }
    },
    [navOpen]
  );

  return (
    <div
      id="fullscreennav"
      ref={fullScreenRef}
      className="z-50 h-screen w-full overflow-hidden absolute "
    >
      <div className="h-screen w-full fixed">
        <div className="h-full w-full flex">
          <div className="stairs h-full w-1/5 bg-black"></div>
          <div className="stairs h-full w-1/5 bg-black"></div>
          <div className="stairs h-full w-1/5 bg-black"></div>
          <div className="stairs h-full w-1/5 bg-black"></div>
          <div className="stairs h-full w-1/5 bg-black"></div>
        </div>
      </div>
      <div ref={fullNavLinksRef} className="relative">
        <div className="navlink flex w-full p-5 justify-between items-start">
          <div className="">
            <div className="w-36">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="103"
                height="44"
                viewBox="0 0 103 44"
              >
                <path
                  fill="white"
                  fill-rule="evenodd"
                  d="M35.1441047,8.4486911 L58.6905011,8.4486911 L58.6905011,-1.3094819e-14 L35.1441047,-1.3094819e-14 L35.1441047,8.4486911 Z M20.0019577,0.000230366492 L8.83414254,25.3433089 L18.4876971,25.3433089 L29.5733875,0.000230366492 L20.0019577,0.000230366492 Z M72.5255345,0.000691099476 L72.5255345,8.44846073 L94.3991559,8.44846073 L94.3991559,16.8932356 L72.5275991,16.8932356 L72.5275991,19.5237906 L72.5255345,19.5237906 L72.5255345,43.9274346 L102.80937,43.9274346 L102.80937,35.4798953 L80.9357483,35.4798953 L80.9357483,25.3437696 L94.3996147,25.3428482 L94.3996147,16.8953089 L102.80937,16.8953089 L102.80937,0.000691099476 L72.5255345,0.000691099476 Z M-1.30398043e-14,43.9278953 L8.78642762,43.9278953 L8.78642762,0.0057591623 L-1.30398043e-14,0.0057591623 L-1.30398043e-14,43.9278953 Z M58.6849955,8.4486911 L43.1186904,43.9274346 L52.3166592,43.9274346 L67.9877996,8.4486911 L58.6849955,8.4486911 Z M18.4688864,25.3437696 L26.7045278,43.9278953 L36.2761871,43.9278953 L28.1676325,25.3375497 L18.4688864,25.3437696 Z"
                ></path>
              </svg>
            </div>
          </div>
          <div
            onClick={() => {
              setNavOpen(false);
            }}
            className="h-20 w-20 relative cursor-pointer"
          >
            <div className="h-28 w-1 -rotate-45 origin-top absolute bg-[#D3FD50]"></div>
            <div className="h-28 w-1 right-0 rotate-45 origin-top absolute bg-[#D3FD50]"></div>
          </div>
        </div>
        <div className="py-10">
          <div className="link origin-top border-t-1 relative">
            <h1 className="font-[font2] text-[6vw] leading-[0.8] pt-3 uppercase text-center">
              Projects
            </h1>
            <div className="moveLink flex absolute top-0  bg-[#D3FD50] text-black">
              <div className="flex items-center  moveX">
                <h2 className="whitespace-nowrap font-[font2] text-[6vw] leading-[0.8] pt-3 uppercase text-center">
                  Pour Tout Voir
                </h2>
                <img
                  className="h-16 w-60 shrink-0 rounded-full object-cover"
                  src="https://k72.ca/images/blog/blogImg/50ff59cc0550df5b36543807a58db98c52e01a22274a317eafbfa5266941579b.png?w=640&h=290&s=4f8134f04fe18db7382b99cec63c95f5"
                  alt=""
                />
                <h2 className="whitespace-nowrap font-[font2] text-[6vw] leading-[0.8] pt-3 uppercase text-center">
                  Pour Tout Voir
                </h2>
                <img
                  className="h-16 w-60 shrink-0 rounded-full object-cover"
                  src="https://k72.ca/images/blog/blogImg/50ff59cc0550df5b36543807a58db98c52e01a22274a317eafbfa5266941579b.png?w=640&h=290&s=4f8134f04fe18db7382b99cec63c95f5"
                  alt=""
                />
              </div>
              <div className="flex items-center moveX ">
                <h2 className="whitespace-nowrap font-[font2] text-[6vw] leading-[0.8] pt-3 uppercase text-center">
                  Pour Tout Voir
                </h2>
                <img
                  className="h-16 w-60 shrink-0 rounded-full object-cover"
                  src="https://k72.ca/images/blog/blogImg/50ff59cc0550df5b36543807a58db98c52e01a22274a317eafbfa5266941579b.png?w=640&h=290&s=4f8134f04fe18db7382b99cec63c95f5"
                  alt=""
                />
                <h2 className="whitespace-nowrap font-[font2] text-[6vw] leading-[0.8] pt-3 uppercase text-center">
                  Pour Tout Voir
                </h2>
                <img
                  className="h-16 w-60 shrink-0 rounded-full object-cover"
                  src="https://k72.ca/images/blog/blogImg/50ff59cc0550df5b36543807a58db98c52e01a22274a317eafbfa5266941579b.png?w=640&h=290&s=4f8134f04fe18db7382b99cec63c95f5"
                  alt=""
                />
              </div>
            </div>
          </div>
          <div className="link origin-top border-t-1 relative">
            <h1 className="font-[font2] text-[6vw] leading-[0.8] pt-3 uppercase text-center">
              Projects
            </h1>
            <div className="moveLink flex absolute top-0  bg-[#D3FD50] text-black">
              <div className="flex items-center  moveX">
                <h2 className="whitespace-nowrap font-[font2] text-[6vw] leading-[0.8] pt-3 uppercase text-center">
                  Pour Tout Voir
                </h2>
                <img
                  className="h-16 w-60 shrink-0 rounded-full object-cover"
                  src="https://k72.ca/images/blog/blogImg/50ff59cc0550df5b36543807a58db98c52e01a22274a317eafbfa5266941579b.png?w=640&h=290&s=4f8134f04fe18db7382b99cec63c95f5"
                  alt=""
                />
                <h2 className="whitespace-nowrap font-[font2] text-[6vw] leading-[0.8] pt-3 uppercase text-center">
                  Pour Tout Voir
                </h2>
                <img
                  className="h-16 w-60 shrink-0 rounded-full object-cover"
                  src="https://k72.ca/images/blog/blogImg/50ff59cc0550df5b36543807a58db98c52e01a22274a317eafbfa5266941579b.png?w=640&h=290&s=4f8134f04fe18db7382b99cec63c95f5"
                  alt=""
                />
              </div>
              <div className="flex items-center moveX ">
                <h2 className="whitespace-nowrap font-[font2] text-[6vw] leading-[0.8] pt-3 uppercase text-center">
                  Pour Tout Voir
                </h2>
                <img
                  className="h-16 w-60 shrink-0 rounded-full object-cover"
                  src="https://k72.ca/images/blog/blogImg/50ff59cc0550df5b36543807a58db98c52e01a22274a317eafbfa5266941579b.png?w=640&h=290&s=4f8134f04fe18db7382b99cec63c95f5"
                  alt=""
                />
                <h2 className="whitespace-nowrap font-[font2] text-[6vw] leading-[0.8] pt-3 uppercase text-center">
                  Pour Tout Voir
                </h2>
                <img
                  className="h-16 w-60 shrink-0 rounded-full object-cover"
                  src="https://k72.ca/images/blog/blogImg/50ff59cc0550df5b36543807a58db98c52e01a22274a317eafbfa5266941579b.png?w=640&h=290&s=4f8134f04fe18db7382b99cec63c95f5"
                  alt=""
                />
              </div>
            </div>
          </div>
          <div className="link origin-top border-t-1 relative">
            <h1 className="font-[font2] text-[6vw] leading-[0.8] pt-3 uppercase text-center">
              Projects
            </h1>
            <div className="moveLink flex absolute top-0  bg-[#D3FD50] text-black">
              <div className="flex items-center  moveX">
                <h2 className="whitespace-nowrap font-[font2] text-[6vw] leading-[0.8] pt-3 uppercase text-center">
                  Pour Tout Voir
                </h2>
                <img
                  className="h-16 w-60 shrink-0 rounded-full object-cover"
                  src="https://k72.ca/images/blog/blogImg/50ff59cc0550df5b36543807a58db98c52e01a22274a317eafbfa5266941579b.png?w=640&h=290&s=4f8134f04fe18db7382b99cec63c95f5"
                  alt=""
                />
                <h2 className="whitespace-nowrap font-[font2] text-[6vw] leading-[0.8] pt-3 uppercase text-center">
                  Pour Tout Voir
                </h2>
                <img
                  className="h-16 w-60 shrink-0 rounded-full object-cover"
                  src="https://k72.ca/images/blog/blogImg/50ff59cc0550df5b36543807a58db98c52e01a22274a317eafbfa5266941579b.png?w=640&h=290&s=4f8134f04fe18db7382b99cec63c95f5"
                  alt=""
                />
              </div>
              <div className="flex items-center moveX ">
                <h2 className="whitespace-nowrap font-[font2] text-[6vw] leading-[0.8] pt-3 uppercase text-center">
                  Pour Tout Voir
                </h2>
                <img
                  className="h-16 w-60 shrink-0 rounded-full object-cover"
                  src="https://k72.ca/images/blog/blogImg/50ff59cc0550df5b36543807a58db98c52e01a22274a317eafbfa5266941579b.png?w=640&h=290&s=4f8134f04fe18db7382b99cec63c95f5"
                  alt=""
                />
                <h2 className="whitespace-nowrap font-[font2] text-[6vw] leading-[0.8] pt-3 uppercase text-center">
                  Pour Tout Voir
                </h2>
                <img
                  className="h-16 w-60 shrink-0 rounded-full object-cover"
                  src="https://k72.ca/images/blog/blogImg/50ff59cc0550df5b36543807a58db98c52e01a22274a317eafbfa5266941579b.png?w=640&h=290&s=4f8134f04fe18db7382b99cec63c95f5"
                  alt=""
                />
              </div>
            </div>
          </div>
          <div className="link origin-top border-y-1 relative">
            <h1 className="font-[font2] text-[6vw] leading-[0.8] pt-3 uppercase text-center">
              Projects
            </h1>
            <div className="moveLink flex absolute top-0  bg-[#D3FD50] text-black">
              <div className="flex items-center  moveX">
                <h2 className="whitespace-nowrap font-[font2] text-[6vw] leading-[0.8] pt-3 uppercase text-center">
                  Pour Tout Voir
                </h2>
                <img
                  className="h-16 w-60 shrink-0 rounded-full object-cover"
                  src="https://k72.ca/images/blog/blogImg/50ff59cc0550df5b36543807a58db98c52e01a22274a317eafbfa5266941579b.png?w=640&h=290&s=4f8134f04fe18db7382b99cec63c95f5"
                  alt=""
                />
                <h2 className="whitespace-nowrap font-[font2] text-[6vw] leading-[0.8] pt-3 uppercase text-center">
                  Pour Tout Voir
                </h2>
                <img
                  className="h-16 w-60 shrink-0 rounded-full object-cover"
                  src="https://k72.ca/images/blog/blogImg/50ff59cc0550df5b36543807a58db98c52e01a22274a317eafbfa5266941579b.png?w=640&h=290&s=4f8134f04fe18db7382b99cec63c95f5"
                  alt=""
                />
              </div>
              <div className="flex items-center moveX ">
                <h2 className="whitespace-nowrap font-[font2] text-[6vw] leading-[0.8] pt-3 uppercase text-center">
                  Pour Tout Voir
                </h2>
                <img
                  className="h-16 w-60 shrink-0 rounded-full object-cover"
                  src="https://k72.ca/images/blog/blogImg/50ff59cc0550df5b36543807a58db98c52e01a22274a317eafbfa5266941579b.png?w=640&h=290&s=4f8134f04fe18db7382b99cec63c95f5"
                  alt=""
                />
                <h2 className="whitespace-nowrap font-[font2] text-[6vw] leading-[0.8] pt-3 uppercase text-center">
                  Pour Tout Voir
                </h2>
                <img
                  className="h-16 w-60 shrink-0 rounded-full object-cover"
                  src="https://k72.ca/images/blog/blogImg/50ff59cc0550df5b36543807a58db98c52e01a22274a317eafbfa5266941579b.png?w=640&h=290&s=4f8134f04fe18db7382b99cec63c95f5"
                  alt=""
                />
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

export default FullScreenNav;
